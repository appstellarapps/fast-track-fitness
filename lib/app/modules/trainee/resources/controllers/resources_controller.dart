...
import '../../trainee_dashboard/trainee_dashboard/controllers/trainee_dashboard_controller.dart';

class ResourcesController extends GetxController {
  //TODO: Implement ResourcesController
  ...
  void onInit() {
    ...
    if (AppStorage.isLogin()) {
      if (AppStorage.userData.result!.user.isResourceSubscribed == 0 &&
          AppStorage.getUserType() == EndPoints.USER_TYPE_TRAINEE) {
        getSubscriptionPlanTrainee();
      }
    }
    apiLoader(asyncCall: () async => getExerciseLibrary());
  }
  ...
  getSubscriptionPlanTrainee() {
    WebServices.postRequest(
      ...
      onStatusSuccess: (res) {
        ...
        hideAppLoader();
        CustomMethod.subscriptionDialog(
            subModel.result!.subscription[0].amount, subModel.result!.subscription[0].validity);
      },
      onFailure: (error) {
        hideAppLoader(hideSnacks: false);
      },
                                ...
                                onPress: () {
                                  ...
                                  Future.delayed(const Duration(milliseconds: 500), () {
                                    if (selectTabIndex.value == 0) {
                                      var ctr2 = Get.put(MyDiaryController());
                                      ctr2.selectTabIndex.value = 0;
                                    } else {
                                      var ctr2 = Get.put(MyDiaryController());
                                      ctr2.selectTabIndex.value = 1;
                                    }
                                  });
                                },
                                color: colorGreyText,
